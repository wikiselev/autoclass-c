...
cluster_CRMs = function (cqbm, hmcol=colorRampPalette(c("#0000fe","black","#fdfd00"), space="Lab")(256), 
                         ...
                         tfrange=7:16, clustexcludenames=NULL, breaks=seq(-1,4,length.out=257), 
                         margins= c(12,3), dist = c("spearman", "euclidian")[2], 
                         clustmethod=c("complete","ward")[2], file="", tree.k = 10,  
...
plot_CRM_heatmap = function(cqbm, hmcol=colorRampPalette(c("#0000fe","black","#fdfd00"), space="Lab")(256), classcol = which(names(cqbm)=="V3"), 							mycol = sample(rainbow(256)), key=TRUE, density.info="none", tfrange=7:16, clustexcludenames=NULL,
							...
							# can also impose limits just using breaks, unless the clustering doesn't look nice this way
       						margins= c(12,3), dist = c("spearman", "euclidian")[2], clustmethod=c("complete","ward")[2], file="", 
       						clustercols=FALSE, coldist = c("spearman", "euclidian")[2], colclustmethod=c("complete","ward")[2],
...
#	}
	...
	cqbm = cqbm[ !cqbm[,classcol]%in%excludeclasses , ]
	
	if (missing(classOrder)){
		classOrder = unique(cqbm[,classcol])
	}
	
	cqbm[, classcol] = factor(cqbm[, classcol], levels=classOrder)
	
	#cat("total len: ",length(cqbm[,classcol]),"\n")

	#print(unique(cqbm[,classcol]))
	
	classes = levels(cqbm[, classcol])
	...
	for (cl in classes ){
		...
		thiscl = as.matrix(
					cqbm[as.factor(cqbm[,classcol])==cl,
					tfrange]
				 )
		...
		hr = hclust(dr, method=clustmethod)
		rowInd = order.dendrogram(as.dendrogram(hr))
				
		thiscl = thiscl[rowInd,]
		
		cat("\tlength thiscl=", length(thiscl[,1]),"\n")		 	
		
		allcl = rbind(allcl, thiscl)
    ...
    mycol = mycol[as.vector(mycl)]        
    heatmap.3(allcl, dendrogram=dgr, Colv=ColV, Rowv=FALSE, trace="none", labRow="", RowSideColors=mycol, col=hmcol, key=key, density.info=density.info, breaks=breaks, margins=margins, rowclnames=classes, separateRows=separateRows, seplcol=seplcol, writeClassNames = writeClassNames, ...)
